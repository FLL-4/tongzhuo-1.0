...
- 现有 3 个 1920x1080 内置 PNG 场景：雨夜书房、湖畔桌面、深夜小屋。
...
flowchart TD
    A[打开同桌] --> B{创建或加入}
    B -- 创建 --> C[Mock 创建固定 Demo 房间]
    B -- 任意非空邀请码 --> D[Mock 加入固定 Demo 房间]
    C --> E[选择专注时长]
    D --> E
    E --> F[选择 1 项未完成 Todo]
    F --> G[从 8 个内置场景随机选择 1 个]
    ...
    K --> M[显示统一留声机引导]
    L --> M
    M -- 打开留声机 --> N[进入现有留声机 Sheet]
    M -- 暂不记录 --> O[关闭引导并保留房间/Todo]
```
...
- 留声机引导复用现有 `PresenceSuggestionView` 视觉样式。
...
- 1920x1080 PNG，16:9。
- 暖色像素风，与现有场景的颗粒、光线和构图密度一致。
- 只包含环境，不包含人物、桌宠、文字 UI 或聊天气泡。
- 中央和底部保留前景桌宠及产品控件的安全区域。
- 为每个场景配置名称、眉题、主文案、可访问性描述、环境声和天气效果。
...
- `MockDeskRoomService` 创建和任意非空码加入同一 Demo 房间。
- 空邀请码被拒绝。
...
- 创建和任意非空邀请码加入均进入同一 Demo 房间。
- 会前必须选择合法时长和一项未完成 Todo。
- 开始专注不最小化、不新开窗口。
- 计时结束和手动结束均只产生一次结束事件，并显示相同留声机引导。
